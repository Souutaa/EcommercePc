...
public interface AccountDetailRepository extends JpaRepository<AccountDetail, Integer> {
    ...
    @Transactional
    @Modifying
    @Query(value = "UPDATE account_detail SET is_default = false WHERE account_id = :accountId", nativeQuery = true)
    void removeAccountDetailsDefaultByAccountId(@Param("accountId") Integer accountId);

    @Transactional
    @Modifying
    @Query(value = "UPDATE account_detail SET is_default = true WHERE id = ?1", nativeQuery = true)
    void setAccountDetailAsDefault(Integer accountDetailId);
